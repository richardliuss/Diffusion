...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            img = th.randn(*shape, device=f'npu:{device}' if isinstance(device, int) else device)
        ...
        for i in indices:
            t = th.tensor([i] * shape[0], device=f'npu:{device}' if isinstance(device, int) else device) 
            with th.no_grad():
                out = self.p_sample(
                    model,
                    img,
    ...
    def _prior_bpd(self, x_start):
        ...
        t = th.tensor([self.num_timesteps - 1] * batch_size, device=f'npu:{x_start.device}' if isinstance(x_start.device, int) else x_start.device)
    ...
    def calc_bpd_loop(self, model, x_start, clip_denoised=True, model_kwargs=None):
        ...
        for t in list(range(self.num_timesteps))[::-1]:
            t_batch = th.tensor([t] * batch_size, device=f'npu:{device}' if isinstance(device, int) else device)
            noise = th.randn_like(x_start)
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    res = th.from_numpy(arr).to(f'npu:{timesteps.device}' if isinstance(timesteps.device, int) else timesteps.device)[timesteps].float()
